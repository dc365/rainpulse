...
.PHONY: bootstrap contracts-generate contracts-check test test-structure test-contracts test-infrastructure test-control-plane test-worker-sdk test-go test-python test-web lint build build-linux build-infrastructure-linux export-postgres-image export-python-image build-worker-linux deploy-up dev-up dev-down smoke infrastructure-smoke control-plane-smoke worker-smoke
...
test: test-structure test-contracts test-infrastructure test-control-plane test-worker-sdk test-go test-python test-web

test-structure:
	bash tests/rp000_structure_test.sh

test-contracts:
	bash tests/rp001_contracts_test.sh
	uv run --project algorithms pytest contracts/tests

test-infrastructure:
	bash tests/rp002_infrastructure_test.sh

test-control-plane:
	bash tests/rp003_control_plane_test.sh

test-worker-sdk:
	bash tests/rp004_worker_sdk_test.sh
