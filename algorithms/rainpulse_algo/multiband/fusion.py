...
def _nearest_ray(
    azimuth: np.ndarray, bearing: np.ndarray, *, prepared=None
) -> tuple[np.ndarray, np.ndarray]:
    order = np.argsort(azimuth) if prepared is None else prepared[0]
    a = azimuth[order] if prepared is None else prepared[1]
...
def beam_height(
    slant_m: np.ndarray, elevation_deg: np.ndarray | float, altitude_m: float
) -> np.ndarray:
    angle = np.deg2rad(elevation_deg)
    return (
        np.sqrt(slant_m**2 + EARTH_EFFECTIVE_M**2 + 2 * slant_m * EARTH_EFFECTIVE_M * np.sin(angle))
        - EARTH_EFFECTIVE_M
        + altitude_m
    )
...
def ground_geometry(station, longitude, latitude):
    ...
    if longitude.size == 1:
        b, _, d = GEOD.inv(
            ...
            float(longitude.item()),
            float(latitude.item()),
        )
        bearing, distance = np.full(longitude.shape, b), np.full(longitude.shape, d)
    else:
        bearing, _, distance = GEOD.inv(
            np.full(longitude.shape, station.longitude_deg),
            np.full(latitude.shape, station.latitude_deg),
            longitude,
            latitude,
        )
...
def prepare_polar(s):
    """Actual per-cut index only; no geometry inferred from band or cut number."""
    order = np.argsort(s.azimuth_deg)
    widths = np.full_like(s.range_m, np.nan, dtype=float)
    if len(s.range_m) >= 2:
        steps = np.diff(s.range_m)
        widths[0], widths[-1] = steps[0], steps[-1]
        widths[1:-1] = np.minimum(steps[:-1], steps[1:])
    return order, s.azimuth_deg[order], widths
...
def footprint(
    ...
    *,
    ...
    bearing, distance = (
        ground if ground is not None else ground_geometry(station, longitude, latitude)
    )
    ray, offset = _nearest_ray(s.azimuth_deg, np.mod(bearing, 360), prepared=prepared)
    ...
    support = (
        ...
        & (age >= 0)
        & (age <= station.maximum_age_seconds)
    )
    h = beam_height(center_range, elevation, station.altitude_m_msl)
    lower = beam_height(
        np.maximum(0, center_range - width / 2),
        elevation - station.beam_width_v_deg / 2,
        station.altitude_m_msl,
    )
    upper = beam_height(
        center_range + width / 2, elevation + station.beam_width_v_deg / 2, station.altitude_m_msl
    )
    resolution = np.maximum(width, center_range * np.deg2rad(station.beam_width_h_deg))
    return Footprint(
        ray, gate, support, h, np.minimum(lower, upper), np.maximum(lower, upper), resolution, age
    )
...
def build_composite(
    ...
    prepared = {id(s): prepare_polar(s) for _, s, _ in sweeps}
    for row in range(0, grid.height, grid.tile_rows):
        stop = min(row + grid.tile_rows, grid.height)
        sl = np.s_[row:stop, :]
        lon, lat = tile_coordinates(grid, transform, row, stop)
        layers = allocate_layers(grid, lon.shape)
        grounds = {}
        for index, sweep, station in sweeps:
            if station.radar_id not in grounds:
                grounds[station.radar_id] = ground_geometry(station, lon, lat)
            fp = footprint(
                sweep,
                station,
                lon,
                lat,
                target,
                ground=grounds[station.radar_id],
                prepared=prepared[id(sweep)],
            )
            update_tile(layers, out, sl, fp, sweep, station, index, grid)
        finish_tile(layers, out, sl, grid)
    return finish_composite(out, grid, network, product, analysis_time, cutoff, sources, skipped)


def tile_coordinates(grid, transform, row, stop):
    x = grid.west_m + (np.arange(grid.width) + 0.5) * grid.spacing_m
    y = grid.south_m + (np.arange(row, stop) + 0.5) * grid.spacing_m
...
def allocate_output(grid):
    ...
    names = (
        ...
        "CR_UNCERTAIN_DBZH",
        "OBSERVED_MIN_HEIGHT_MSL_M",
        "OBSERVED_MAX_HEIGHT_MSL_M",
    )
    out = {name: np.full(shape, np.nan, np.float32) for name in names}
    out.update(
        {
            name: np.full(shape, -1, np.int32)
            for name in ("WINNER_SOURCE", "WINNER_RAY", "WINNER_GATE", "COVERAGE_SOURCE")
        }
    )
...
def allocate_layers(grid, tile_shape):
...
def update_tile(layers, out, sl, fp, sweep, station, index, grid, *, backend="numpy"):
    ...
    for li, level in enumerate(grid.levels_m_msl):
        ...
        out["CR_UNCERTAIN_DBZH"][sl] = np.fmax(
            out["CR_UNCERTAIN_DBZH"][sl], np.where(uncertain, sample, np.nan)
        )
...
def finish_tile(layers, out, sl, grid):
    ...
    for li, level in enumerate(grid.levels_m_msl):
        ...
        minimum, maximum = (
            out["OBSERVED_MIN_HEIGHT_MSL_M"][sl],
            out["OBSERVED_MAX_HEIGHT_MSL_M"][sl],
        )
        ...
        replace = (
            covered
            & np.isfinite(values[li])
            & (~np.isfinite(out["CR_DBZH"][sl]) | (values[li] > out["CR_DBZH"][sl]))
        )
        for key, data in (
            ("CR_DBZH", values[li]),
            ("WINNER_SOURCE", winner[li]),
            ("WINNER_RAY", wray[li]),
            ("WINNER_GATE", wgate[li]),
            ("WINNER_HEIGHT_MSL_M", h[li]),
            ("WINNER_AGE_SECONDS", age[li]),
            ("WINNER_RESOLUTION_M", resolution[li]),
            ("WINNER_QUALITY_SCORE", score[li]),
        ):
            out[key][sl][replace] = data[replace]


def finish_composite(out, grid, network, product, analysis_time, cutoff, sources, skipped):
    ...
    meta = {
        "contract": "rainpulse.multiband.composite-v1",
        "product_id": product,
        ...
        "levels_m_msl": grid.levels_m_msl,
        "analysis_time": analysis_time,
        "input_cutoff": cutoff,
        "cadence_seconds": grid.cadence_seconds,
        "network_release": network.release_id,
        "network_sha256": network.sha256,
        "sources": sources,
        "skipped": skipped,
        "method": "quality_select_at_height_then_vertical_max_v1",
        "temporal_method": "causal_hold_with_measured_age_no_advection",
        "vertical_coverage": "observed_levels_only",
        ...
        "uncertain_only_cells": int(
            np.count_nonzero(~np.isfinite(out["CR_DBZH"]) & (out["UNCERTAIN_MASK"] == 1))
        ),
        "missing_cells": int(
