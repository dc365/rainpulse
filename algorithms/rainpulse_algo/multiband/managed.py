...
class VolumeCache:
    def __init__(self, max_bytes: int, ttl_seconds: int, *, clock: Callable[[], float] = time.monotonic):
    ...
    def put(self, key: tuple, value: Volume):
        ...
        while self.entries and (self.bytes+value.nbytes > self.maximum or len(self.entries) >= 32):
            self.bytes -= self.entries.popitem(last=False)[1][1].nbytes
        for s in value.sweeps:
            for a in [s.azimuth_deg, s.range_m, s.elevation_deg, s.ray_time_epoch, *s.fields.values()]:
...
class Executor:
    def __init__(self, path: str | Path, *, reject_mask: int = 0, flag_version: str = ""):
        self.path = Path(path)
    ...
    def execute(self, request: dict, reader, *, artifact_digest: Callable[[dict], str]):
        ...
        if request.get("event_type") != "ops.multiband.requested.v1" or p["mode"] not in {"x_qc", "sx_composite"}:
        ...
        for source in inputs:
            ...
            key = (source["input_uri"], station.radar_id, source["scan_id"], sha, self.network.sha256, source["available_at"], source["volume_start"], source["volume_end"], self.flag_version, self.reject_mask)
            ...
            if v is None:
                if session is not None:
                    ...
                    read_ms += (time.perf_counter()-mark)*1000
                mark = time.perf_counter()
                v = read_volume(objects, station, source, asset_sha256=sha, maximum_bytes=self.network.maximum_input_bytes,
                                s_reject_mask=self.reject_mask, expected_flag_version=self.flag_version)
                v.metadata["input_uri"] = source["input_uri"]
                v = x_qc(v, station, self.network.sha256) if station.band == "X" else accept_s_qc(v, station, self.network.sha256)
        ...
        mark = time.perf_counter()
        composite = build_composite(volumes, self.network, p["product_id"], p["analysis_time"], p["input_cutoff"])
        ...
        summary = {"multiband": {k: composite.metadata[k] for k in ("product_id", "analysis_time", "network_release", "network_sha256", "valid_echo_cells", "valid_no_echo_cells", "missing_cells")},
                   "candidate_only": True, "operational_eligible": False, "qpe_enabled": False,
                   "detail": "manifest.json", "mode": p["mode"]}
        metrics = {"input_read_ms": read_ms, "station_qc_ms": qc_ms, "fusion_ms": fusion_ms,
                   "total_ms": (time.perf_counter()-started)*1000, "resident_input_bytes": float(sum(v.nbytes for v in volumes)),
                   "decoded_cache_bytes": float(self.cache.bytes), "decoded_cache_hits": float(self.cache.hits),
                   "decoded_cache_misses": float(self.cache.misses), "output_object_count": float(len(objects))}
    ...
    for key in keys:
        ...
        if key in {".zattrs", ".zgroup"} or parts[0] in root_arrays:
            ...
            if len(parts) == 2 and parts[1] in {".zattrs", ".zgroup"}:
                selected.append(key)
            elif len(parts) >= 3 and (parts[1] in FIELDS | coordinates or parts[1].endswith("CR_WITHHELD_MASK")):
                selected.append(key)
...
def existing_runtime_executor() -> Executor:
    ...
    return Executor(os.environ["RAINPULSE_MULTIBAND_CONFIG"], reject_mask=mask, flag_version=value["definition_version"])
