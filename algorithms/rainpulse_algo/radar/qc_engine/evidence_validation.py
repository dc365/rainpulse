...
def validate_evidence_fields(group, observed, reject):
    ...
    p2_added = np.zeros(observed.shape, bool)
    if "P2_ADDED_QUARANTINE_MASK" in group:
        for name in (
            ...
            if (
                ...
                raise ValueError("invalid P0-P2 action provenance")
        p2_added = mask("P2_ADDED_QUARANTINE_MASK")
        p2_base = mask("P2_BASELINE_QUARANTINE_MASK")
        if np.any(p2_added & (~observed | reject | p2_base | ~mask("P2_PROPOSAL_MASK"))):
            raise ValueError("P0-P2 additions lack measured candidate support")
        if not np.array_equal(quarantine, p2_base | p2_added):
            raise ValueError("P0-P2 quarantine is not baseline plus additions")
        if np.any(mask("QPE_ELIGIBLE_MASK") & ~mask("P2_BASELINE_ELIGIBLE_MASK")):
            raise ValueError("P0-P2 restored pre-stage ineligible measurements")
        quarantine = p2_base
    ...
    if "OC1_ADDED_QUARANTINE_MASK" in group:
        ...
        if np.any(oc_added & (~observed | reject | oc_base)) or not np.array_equal(
            quarantine, oc_base | oc_added
        ):
    ...
    return domain | oc_added | p2_added
