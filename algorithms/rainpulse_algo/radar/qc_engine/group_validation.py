...
def _groups(oid, selected):
    ...
    if (flat.dtype.kind in "iu" and flat.size and flat.min() >= 0
...
def validate_seed_objects(oid, sizes, fractions, core, propagated, *,
                          maximum_object_gates, minimum_seed_gates,
    ...
    mismatch = np.bincount(
        inverse[~np.isclose(np.asarray(fractions).ravel(), expected[inverse],
                           rtol=1e-5, atol=1e-8, equal_nan=False)], minlength=len(keys)
    ) > 0
    ...
    if np.any(wanted & ((counts > maximum_object_gates) |
...
def validate_temporal_objects(oid, sizes, fractions, selected, *, minimum_gates,
                              maximum_gates, minimum_recurrence_fraction):
    ...
    if np.any(wanted & ((counts < minimum_gates) | (counts > maximum_gates) |
