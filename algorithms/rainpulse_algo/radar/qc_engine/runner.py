...
def run_open_source_qc(
    ...
    **kwargs,
    ...
    for sweep in native:
        ...
        first = decide(sweep, evidence, profile, rfi_candidate=radial, clutter_prior=prior)
        independent.append((evidence, radial, radial_record, first, prior))
    ...
    for index, (sweep, item) in enumerate(zip(native, independent, strict=True)):
        evidence, radial, radial_record, _, prior = item
