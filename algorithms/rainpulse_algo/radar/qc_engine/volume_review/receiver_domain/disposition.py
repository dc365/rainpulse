...
def apply(group, evidence, cfg, *, low_quality_flag):
    ...
    partial_action = partial & ~hard & ~local & (evidence["RDR_TARGET_SIDE_CONFLICT_MASK"] == 0) & (cfg.partial_policy == "cr_withhold") & active
    selected = (source & active) | partial_action
    ...
    q = source & trust & obs & (a["QC_ACTION"] != 2) & (cfg.mode == "quarantine")
    loss = cr & selected
    ...
    a[CR][loss] = 0
    if active:
        a["CR_UNCERTAIN_MASK"][partial | (evidence["RDR_MIXED_MASK"] == 1)] = 1
