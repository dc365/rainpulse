...
def common_metrics(obs, forecasts, bounds):
    ...
    return {name:spatial_metrics(truth,np.where(common,field,np.nan),bounds)
...
def spectrum(fields, bounds):
    ...
    for size in [256,128,64,32]:
        ...
        if len(candidates):
            ...
            break
    if crop is None:
        return dict(status='unavailable',reason='共同有效域内没有至少 32×32 格的完整区域，无法可靠计算 PSD')
    y,x,n = crop
    ...
    lower,upper = max(1/(n*dx),1/(n*dy)),min(.5/dx,.5/dy)
    edges = np.geomspace(lower,upper,17)
    bins = [(radial >= a) & (radial < b if i < 15 else radial <= b)
    ...
    for name,field in fields.items():
        ...
        power = np.abs(np.fft.fft2((data-data.mean())*window))**2 * dx*dy / np.sum(window**2)
        series[name] = [float(power[b].mean()) for b in bins]
    return dict(status='ready',scale_km=scales,series=series,shape=[n,n],
                bounds=[west+x*dlon,south+y*dlat,west+(x+n)*dlon,south+(y+n)*dlat],
                coverage=n*n/(height*width),unit='(mm/h)^2 km^2',method='demean-hann-2d-radial-density-v1')


def compare(service, request):
    key = hashlib.sha256(json.dumps(['comparison-v1',request],sort_keys=True).encode()).hexdigest()
    with service.compute_lock:
        ...
        forecasts = {name:forecast_field(service,dict(algorithm=name,forecast=sources,
            bounds=bounds,lead_minutes=request['lead_minutes'])) for name,sources in request['forecasts'].items()}
...
def summarize(records, algorithms):
    ...
    def aggregate(rows):
        ...
        for name in names:
            paired = [r for r in rows if all(isinstance(r.get('metrics',{}).get(a,{}).get(name),(int,float))
                      and np.isfinite(r['metrics'][a][name]) for a in algorithms)]
            for a in algorithms:
                values = [r['metrics'][a][name] for r in paired]
                result[a][name] = dict(mean=float(np.mean(values)) if values else None,n=len(values))
        return result
    return dict(overall=aggregate(ready),by_lead=[dict(lead_minutes=lead,algorithms=aggregate(
        [r for r in ready if r['lead_minutes']==lead])) for lead in sorted({r['lead_minutes'] for r in records})],
        matched_records=len(ready),skipped_records=len(records)-len(ready),method='matched-macro-mean-v1')
