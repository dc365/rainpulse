...
from .mrms_ensemble_profile import (
...
def evaluate_ensemble_summary(
    ...
    *,
    ...
    persistence = wet_near["steps_skill_against_deterministic_baselines"]["persistence"]
    ...
    checks = {
        ...
        "near_common_verification_coverage": (
            (_number(near["minimum_common_verification_coverage"]) or 0.0)
            >= float(criteria["near_minimum_common_verification_coverage"])
        ),
        "near_steps_member_mean_coverage": (
            (_number(near["minimum_steps_member_mean_coverage"]) or 0.0)
            >= float(criteria["near_minimum_steps_member_mean_coverage"])
        ),
        "far_common_verification_coverage": (
            (_number(far["minimum_common_verification_coverage"]) or 0.0)
            >= float(criteria["far_minimum_common_verification_coverage"])
        ),
        "far_steps_member_mean_coverage": (
            (_number(far["minimum_steps_member_mean_coverage"]) or 0.0)
            >= float(criteria["far_minimum_steps_member_mean_coverage"])
        ),
        "near_wet_crps_skill_against_persistence": (
            _number(persistence["crps_skill"]) is not None
            and float(persistence["crps_skill"])
            > float(criteria["near_wet_minimum_crps_skill_against_persistence"])
        ),
        "near_wet_finite_brier_skill_threshold_count": (
            len(finite_brier)
            >= int(criteria["near_wet_minimum_finite_brier_skill_threshold_count"])
        ),
        "near_wet_mean_brier_skill_against_persistence": (
    ...
    return {
        ...
        "observed": {
            "completed_issue_count": int(summary["completed_issue_count"]),
            "failed_issue_count": int(summary["failed_issue_count"]),
            "near_minimum_common_verification_coverage": near[
                "minimum_common_verification_coverage"
            ],
            "near_minimum_steps_member_mean_coverage": near[
                "minimum_steps_member_mean_coverage"
            ],
            "far_minimum_common_verification_coverage": far[
                "minimum_common_verification_coverage"
            ],
            "far_minimum_steps_member_mean_coverage": far[
                "minimum_steps_member_mean_coverage"
            ],
            "near_wet_crps_skill_against_persistence": persistence["crps_skill"],
...
def build_development_gate_artifact(
    ...
    *,
    ...
    if profile.gate.status != "development_pending":
        raise ValueError("RP-024 development gate has already been frozen")
    if (
        ...
        or int(development_summary.get("member_count", -1)) != profile.member_count
    ):
        raise ValueError("RP-024 development summary does not match the pending profile")
    evaluation = evaluate_ensemble_summary(
        development_summary,
    ...
    if not evaluation["passed"]:
        ...
        raise ValueError(f"RP-024 development gate failed: {', '.join(failed)}")
    return {
        "schema_version": "1.0",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Freeze the RP-024 pre-holdout gate")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        profile = load_mrms_ensemble_profile(profile_path, repository_root=repository_root)
        ...
        if not summary_path.is_relative_to(repository_root):
            raise ValueError("RP-024 development summary must remain repository-local")
        ...
        if not output_path.is_relative_to(repository_root):
            raise ValueError("RP-024 gate artifact must remain repository-local")
