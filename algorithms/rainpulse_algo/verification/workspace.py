...
def spatial_metrics(observed, predicted, bounds):
    ...
    for threshold in [1,5,10,20,50]:
        ...
        for km in [1,5,10,20,40]:
            ...
            size = (ny,nx)
            support = uniform_filter(valid.astype(float), size=size, mode='constant', cval=0) > 1-1e-10
            ...
            union = int((oe | pe).sum())
            rows.append(dict(threshold=threshold, window_km=km, window_shape=[ny,nx],
                actual_km=[nx*dx,ny*dy], neighborhood_cells=int(support.sum()),
                hits=hits, misses=misses, false_alarms=false_alarms, csi=csi,
                fss=max(0.,min(1.,1-float(np.sum((of-pf)**2))/denom)) if denom > 1e-20 else None,
                neighborhood_csi=int((oe & pe).sum())/union if union else None))
    return dict(valid_cells=count, total_cells=valid.size, coverage=count/valid.size,
                mae=float(np.mean(np.abs(difference))) if count else None,
                rmse=float(np.sqrt(np.mean(difference**2))) if count else None, rows=rows)
...
def point_field(service, source, bounds, algorithm):
    ...
    legacy = [x,y,float(np.float32(x+dx))-float(np.float32(x)),float(np.float32(y+dy))-float(np.float32(y))]
...
def forecast_field(service, request):
    from rainpulse_algo.worker.object_store import ArtifactObjectReader, artifact_sha256, minio_client_from_environment
    ...
    if not np.allclose(lon, bounds[0]+(np.arange(len(lon))+.5)*dx, rtol=0, atol=1e-5) or not np.allclose(lat, bounds[1]+(np.arange(len(lat))+.5)*dy, rtol=0, atol=1e-5):
