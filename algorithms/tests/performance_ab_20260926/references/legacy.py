...
PALETTE = [[5,"#419bf1"],[10,"#64e8ec"],[15,"#6efb3d"],[20,"#00dc00"],
 [25,"#019000"],[30,"#fdfe00"],[35,"#e7c000"],[40,"#ff9000"],[45,"#fe0000"],
 [50,"#d60000"],[55,"#c00000"],[60,"#ff00f0"],[65,"#9500b4"],[70,"#ae90f0"]]
...
def polar_quicklook(azimuth_deg, range_m, values, *, uncertain=None, actions=None,
                    size=720, map_sampling=None, elevation_deg=None):
    ...
    order=np.argsort(az);ordered=az[order]
    ...
    left=np.clip(right-1,0,len(extended)-1);right=np.clip(right,0,len(extended)-1)
    ...
    angular_limit=min(max(float(np.median(circular_steps[circular_steps>0]))*1.5,0.1)
                      if np.any(circular_steps>0) else 0.1,MAX_PPI_INTERPOLATION_GAP_DEG)
    if map_sampling is not None:
        re=6371008.8*4/3;angle=distance/re
    ...
    if actions is not None:
        ...
        rejected=(action==2)&valid;uncertain_gate=(action==3)&valid
...
def map_sampling(az,el,r,size):
    geod=Geod(ellps="WGS84");lon,lat=119.,26.
    re=6371008.8*4/3;e=np.deg2rad(el)
    ...
    xs=x.min()+(np.arange(size)+.5)*(x.max()-x.min())/size
    ys=y.min()+(np.arange(size)+.5)*(y.max()-y.min())/size
    xx,yy=np.meshgrid(xs,ys)
    a,_,d=geod.inv(np.full_like(xx,lon),np.full_like(yy,lat),xx,yy)
    return d,a%360

def loop_validate(oid,size,fraction,core,propagated,maximum=5000,min_seeds=3,min_fraction=.3):
