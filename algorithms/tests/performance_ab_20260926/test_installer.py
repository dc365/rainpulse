...
def test_transform_rejects(bad):
    ...
    r = (
        ...
        if bad == "missing"
        else {"replacements": [["return 1", "return ("]]}
        if bad == "syntax"
        else {"replacements": [["return 1", "return 2", 2]]}
    )
...
def test_full_apply_rollback(tmp_path):
    ...
    new = ins.transform(old, {"decorators": {"f": {"label": "function"}}})
...
def test_path_refusal(tmp_path, path):
    with pytest.raises(ValueError):
        ins.safe(tmp_path, path)


def test_symlink_refusal(tmp_path):
    (tmp_path / "a").symlink_to(tmp_path, target_is_directory=True)
    with pytest.raises(ValueError):
        ins.safe(tmp_path, "a/file.py")


def test_package_plan_hashes_and_complete_diff(tmp_path):
    root = repo(tmp_path)
    package = tmp_path / "pkg"
    (package / "recipes").mkdir(parents=True)
    (package / "payload").mkdir()
    head = subprocess.check_output(["git", "-C", str(root), "rev-parse", "HEAD"], text=True).strip()
    old = (root / "base.py").read_bytes()
    recipe = json.dumps(
        {"base.py": {"git_blob": ins.blob(old), "decorators": {"f": {"label": "work"}}}}
    ).encode()
    (package / "recipes/changes.json").write_bytes(recipe)
    (package / "payload/new.py").write_bytes(b"x=1\n")
    manifest = {
        "schema": "rainpulse.performance-ab-delivery-v1",
        "base_commit": head,
        "package_id": "test",
        "recipes_sha256": ins.sha(recipe),
        "payloads": {"new.py": ins.sha(b"x=1\n")},
    }
