...
def test_warmup_real_layouts(dtype, layout, tmp_path):
    ...
    with perf.capture_traces(lambda _: None):
        k.warmup("numba")
    count = len(k.require_numba().signatures)
    shape = (8, 13)
    rng = np.random.default_rng(223)
    b = rng.random(shape) > 0.2
    f = rng.random(shape)
    z = rng.uniform(-20, 70, shape).astype(dtype)
    i = rng.integers(0, 20, shape, dtype=np.int64)
    if layout == "contiguous":
        state = (
            np.full(shape, -np.inf),
            np.full(shape, np.nan, "f4"),
            *[np.full(shape, -1, "i4") for _ in range(3)],
            *[np.full(shape, np.inf, "f4") for _ in range(3)],
        )
    else:
        record = (
            np.zeros(shape, dtype=STATE_DTYPE)
            if layout == "structured"
            else np.memmap(tmp_path / "test.bin", mode="w+", dtype=STATE_DTYPE, shape=shape)
        )
        state = tuple(record[n] for n in STATE_DTYPE.names)
        state[0][:] = -np.inf
...
def test_observes_new_layout_compile():
    pytest.importorskip("numba")
    reports = []
    with perf.capture_traces(reports.append):
        k.warmup("numba")
