...
def test_nested_inclusive_self_and_no_result_mutation():
    seen=[]
    @p.timed('child')
    def child():time.sleep(.001);return {'bytes':b'unchanged'}
    @p.timed('root',root=True)
    def root():
        p.observe('objects',3);time.sleep(.001);return child()
    with p.capture_traces(seen.append):out=root()
    assert out=={'bytes':b'unchanged'} and len(seen)==1
    report=seen[0];st=report['stages']
    ...
    assert 'scope' in report['resources'] and report['resources']['sample_count']>=1
...
def test_parallel_context_union():
    reports=[]
    @p.timed('download')
    def task():time.sleep(.004);p.observe('bytes',10)
    ...
    st=reports[0]['stages'];assert st['root/download']['count']==3
...
def test_async_to_thread_context():
    ...
    async def main():return await asyncio.to_thread(call)
    with p.capture_traces(reports.append):assert asyncio.run(main())==3
...
def test_generator_only_resumes_are_measured():
    ...
    with p.capture_traces(reports.append):
        with p.measure('root',root=True):
            it=source();assert next(it)==1;time.sleep(.01);assert it.send(None)==2
            with pytest.raises(StopIteration):next(it)
            it.close()
    root=reports[0]['stages']['root'];part=reports[0]['stages']['root/read']
    assert part['errors']==0 and root['inclusive_ms']-part['inclusive_ms']>5


def test_exception_still_same_and_no_secrets():
    reports=[];err=ValueError('do-not-serialize-this-secret')
    @p.timed('op',root=True)
    def call(claim):raise err
    claim={'task_id':'x','attempt_id':'a','token':'private-token','identity':{'fingerprint':'abc','token':'private'},
           'request':{'job_id':'j','event_type':'ops.multiband.requested.v1','payload':{'password':'secret'}}}
    with p.capture_traces(reports.append):
        with pytest.raises(ValueError) as exc:call(claim)
    assert exc.value is err
    text=json.dumps(reports)
    assert 'private' not in text and 'password' not in text and str(err) not in text
    assert reports[0]['call_status']=='raised' and reports[0]['identity']['frozen_fingerprint']=='abc'


def test_disabled_does_not_emit(monkeypatch):
    monkeypatch.setenv('RAINPULSE_PERFORMANCE_TELEMETRY','0');seen=[]
    @p.timed('root',root=True)
    def root():return 42
    with p.capture_traces(seen.append):assert root()==42
...
def test_telemetry_failure_cannot_fail_work(monkeypatch):
    @p.timed('root',root=True)
    def call():return b'x'
    def fail(*a,**kw):raise RuntimeError('sink down')
    monkeypatch.setattr(p,'_emit',fail);assert call()==b'x'
    monkeypatch.setattr(p,'resource_snapshot',fail);assert call()==b'x'


def test_bounded_records(monkeypatch):
    seen=[];monkeypatch.setattr(p,'MAX_STAGES',12);monkeypatch.setattr(p,'MAX_CHILD_INTERVALS',5)
    with p.capture_traces(seen.append):
        with p.measure('root',root=True):
            for i in range(40):
                with p.measure('child'+str(i)):pass
...
def test_signature_and_dict_identity():
    import inspect
    @p.timed('root',root=True)
    def call(x:int,*,output=None):return x
    assert str(inspect.signature(call))=='(x: int, *, output=None)'
    seen=[]
    with p.capture_traces(seen.append):
        call({'event_type':'ops','payload':{'network_sha256':'a'*64,'execution_sha256':'b'*64,'mode':'x_qc','raw_password':'secret'}})
...
def test_isolated_parallel_roots():
    ...
    def task(x):
        p.identify(job_id=x);p.observe('sum',x);time.sleep(.002);return x
...
def test_cancellation_closes_trace():
    ...
    async def call():raise asyncio.CancelledError()
    with p.capture_traces(seen.append):
...
def test_counter_limit_and_nonfinite(monkeypatch):
    seen=[];monkeypatch.setattr(p,'MAX_COUNTERS',3)
    with p.capture_traces(seen.append):
        with p.measure('root',root=True):
            p.observe('nan',float('nan'));p.observe('bool',True)
            for i in range(10):p.observe(str(i),i)
...
def test_generator_throw_preserves_cleanup():
    ...
    def source():
        try:
            ...
            yield 2
        finally:clean.append(True)
    with p.capture_traces(seen.append):
        with p.measure('root',root=True):
            it=source();assert next(it)==1;assert it.throw(KeyError('x'))==2;it.close()
    assert clean==[True]
...
def test_request_identity_and_age_do_not_mutate_metrics():
    ...
    reports=[]
    request={'event_type':'test.task','job_id':'job-1','run_id':'run-1',
    ...
    result=SimpleNamespace(observability={'input_bytes':123,'decoded_cache_hits':7,
                                        'resident_input_bytes':456,'password':'hidden'},
                           objects={'manifest.json':b'unchanged'})
    @p.timed('test.identity',root=True)
    def call(value):return result
    with p.capture_traces(reports.append):out=call(request)
...
def test_resource_unavailable_is_not_zero(monkeypatch):
    monkeypatch.setattr(p,'resource_snapshot',lambda:{'gauges':{},'counters':{},'scope':'unavailable'})
    reports=[]
    with p.capture_traces(reports.append):
        with p.measure('no-resource',root=True):pass
...
def test_actual_release_keys():
    from types import SimpleNamespace
    w=SimpleNamespace(_startup_release_identity={'qc_config_sha256':'a'*64,
         'qc_flags_sha256':'b'*64,'runtime_sha256':'c'*64,'profile':'radar-qc-basic',
         'secret':'hidden'})
...
def test_generator_type_and_return_value():
    ...
    iterator=call();assert inspect.isgenerator(iterator) and next(iterator)==3
    with pytest.raises(StopIteration) as e:next(iterator)
...
def test_request_age_first_observation_only():
    ...
    with p.capture_traces(reports.append):
        with p.measure('root',root=True):
            p._event_age(datetime.now(timezone.utc)-timedelta(seconds=1))
            p._event_age(datetime.now(timezone.utc)-timedelta(days=1))
    assert 1<=reports[0]['counters']['request_age_at_start_seconds']<5


def test_frozen_identity_does_not_overwrite_running_identity():
    from types import SimpleNamespace
    actual=SimpleNamespace(network=SimpleNamespace(sha256='a'*64),
           adapter=SimpleNamespace(startup={'code_sha256':'c'*64,'fingerprint':'d'*64}))
    claim={'task_id':'x','attempt_id':'y','request':{},
