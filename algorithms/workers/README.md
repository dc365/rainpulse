...
`nowcast-input` consumes only atomically committed RadarAnalysis v1.2 frames.
It requires an exact five-minute sequence ending at issue time, one immutable
grid and QPE version, complete masks and provenance, and the versioned
coverage/QI/data-age gates. It publishes the canonical `time × lat × lon`
NowcastInput v1.2 without filling missing cells as zero rainfall.
