...
const XQCWorkspace = lazy(() => import('./workspace/XQCWorkspace').then(m => ({ default: m.XQCWorkspace })))
...
export default function App() {
  ...
  const content = window.location.pathname === '/qc-review' ? <QCReviewWorkspace />
    ...
    : new URLSearchParams(window.location.search).get('band') === 'X' ? <XQCWorkspace /> : <MainWorkspace />
