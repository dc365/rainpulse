...
function AdminShell({ token, validationMode, view, navigate, logout }: {
    ...
    const title = { tasks: '任务中心', new: '新建重算', workers: '执行资源', logs: '系统日志', data: '数据台账', performance: '性能分析', storage: '算法与存储' }[view.page];
    return <div className="ops-app"><aside className="ops-sidebar"><a className="ops-brand" href="/admin"><span className="ops-brand-mark">R</span><div>RainPulse<small>运行管理平台</small></div></a><nav aria-label="后台导航">{([['tasks', '任务中心', '01'], ['data', '数据台账', '02'], ['new', '新建重算', '03'], ['workers', '执行资源', '04'], ['performance', '性能分析', '05'], ['logs', '系统日志', '06'], ['storage', '算法与存储', '07']] as const).map(([page, label, n]) => <button key={page} className={view.page === page ? 'selected' : ''} onClick={() => navigate({ page })}><span>{n}</span>{label}</button>)}</nav><div className="ops-sidebar-bottom"><p>候选计算 · 不覆盖业务结果</p><a href="/">← 天气工作台</a>{validationMode ? <span>免凭据验证模式</span> : <button onClick={logout}>退出管理会话</button>}</div></aside>
 ...
 {health.data && !health.data.database_ready && <Notice error>管理表尚未初始化或未升级。请按部署说明依次运行 operations/schema.sql 、schema_v2.sql 和 schema_v3.sql；天气工作台不受影响。</Notice>}
 ...
 {view.page === 'performance' && <Performance token={token} navigate={navigate}/>}
 {view.page === 'storage' && <Maintenance token={token} navigate={navigate}/>}
