...
export function NewRun({ token, navigate, initialJob, initial }: {
    ...
    return <><div className="ops-heading"><div><p className="ops-eyebrow">PLAN BEFORE EXECUTION</p><h1>新建重算</h1><p>先检查资料、配置和 Worker，再提交不可变的执行计划。</p></div></div><Notice>支持既有 QC／图件重算，以及 X 单站质控、S/X 六分钟组合反射率候选。X QC 可查看同一扫描前后对照；融合任务产出 S 单独、X 单独、S/X 与共同有效像元差值快视图。多波段需要已发布的网络配置与 Worker，不改变默认业务产品。</Notice><div className="ops-plan-layout"><form className="ops-panel ops-form" onSubmit={e => void check(e)}><h2>1. 选择范围</h2><label>重算预设<select value={preset} disabled={busy} onChange={e => { setPreset(e.target.value); clearPlan(); }}><option value="qc_preview">单站 QC 与原始／质控对照图</option><option value="render_only">仅重建已有 QC 的对照图</option><option value="diagnostics">重建已有区域诊断任务</option><option value="x_qc">X 波段单站 QC 与诊断</option><option value="sx_composite">S/X 六分钟融合组合反射率</option></select></label>{preset === 'sx_composite' && <label>产品 ID<input value={productID} onChange={e => { setProductID(e.target.value); clearPlan(); }} placeholder="网络配置只有一个产品时可留空" maxLength={96}/><small>融合站点、网格、高度层和衰减策略由已冻结的网络发布文件决定。</small></label>}<label>作业名称<input placeholder="例如：ZF101 质控样本复核" maxLength={120} value={name} onChange={e => { setName(e.target.value); clearPlan(); }}/></label>{preset === 'diagnostics' ? <label>源区域诊断任务 ID<textarea rows={4} placeholder="从自动链路任务详情复制；最多32个，以逗号分隔" value={jobs} onChange={e => { setJobs(e.target.value); clearPlan(); }}/></label> : <><label>雷达站号<input placeholder="例如 zf101" value={radars} onChange={e => { setRadars(e.target.value); clearPlan(); }} required/></label><div className="ops-form-row"><label>开始时间（UTC+8，含）<input type="datetime-local" value={start} onChange={e => { setStart(e.target.value); clearPlan(); }} required/></label><label>结束时间（UTC+8，不含）<input type="datetime-local" value={end} onChange={e => { setEnd(e.target.value); clearPlan(); }} required/></label></div><p className="ops-caption">{multiband ? (preset === 'x_qc' ? 'X QC 使用原生站心极坐标，不要求空间几何；每个计划最多8个真实扫描。S/X 融合按六分钟时次选因果体扫，展示同网格下 S、X、融合与差值候选。' : '每次不超过1小时、64个融合时次；S/X 融合选择结束时间早于六分钟时次的体扫。X 站只有通过几何和标定门控才参与空间组合。这里创建历史候选任务，不是自动实时开关。') : '单次不超过24小时、64个体扫。目录读取不依赖成功的预报产品；大范围请分段执行。'}</p><button type="button" disabled={busy} onClick={listInputs}>查看此范围数据台账</button></>}{error && <Notice error>{error}</Notice>}<button className="ops-primary" disabled={busy}>{busy ? '检查／提交处理中…' : '2. 生成预检查计划'}</button></form><section className="ops-panel ops-plan"><h2>2. 预检查与影响</h2>{plan ? <><p><code>{plan.id}</code><Copy value={plan.id}/></p><Notice>{plan.impact}</Notice><p className="ops-caption">计划中的选择：{plan.selection.name || '未命名'} · {plan.selection.preset} · {plan.tasks.length} 个阶段任务。表单变化后必须重新检查。</p><ul className="ops-checklist">{plan.checks.map((c, i) => <li key={`${c.code}-${i}`}><Badge state={c.state}/><div>{c.message}{c.target && <code>{c.target}</code>}</div></li>)}</ul><details><summary>将要执行的任务与依赖</summary>{plan.tasks.map(t => <p key={t.id}>{t.parent_id ? '↳ ' : ''}{t.name}</p>)}</details><p>计划有效至 {timestamp(plan.expires_at)} {Date.parse(plan.expires_at) < now && <Badge state="BLOCK">已过期，请重新检查</Badge>}</p><Download value={plan} name="rerun-plan.json" label="导出冻结计划"/><button className="ops-primary" disabled={busy || !plan.submittable || Date.parse(plan.expires_at) < now} onClick={() => void submit()}>3. 确认并提交此计划</button><button disabled={busy} onClick={clearPlan}>丢弃此计划</button></> : <Empty>选择范围后生成计划。缺资料、没有匹配 Worker 或配置漂移会明确阻止提交，不会悄悄跳过。</Empty>}</section></div>
