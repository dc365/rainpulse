...
function TaskDrawer({ token, id, onClose, onChange }: {
 ...
 {tab === 'results' && <div className="ops-drawer-body">{task.storage_state && task.storage_state !== 'AVAILABLE' ? <Notice>候选数据{task.storage_state === 'DELETED' ? '已清理' : '已退役'}；下方历史任务状态不代表数据仍然存在。日志与输入配置仍可查看。</Notice> : task.result ? <><Notice>候选产物已登记；没有替换当前天气工作台产品，也没有提升业务资格。</Notice><code className="ops-long">{task.result.asset.uri}</code><p>{bytes(task.result.asset.size_bytes)} · 计算至编码口径 {duration(task.result.runtime_ms)}</p><JSONView value={task.result} label="结果与来源摘要"/>{task.spec.kind !== 'qc' ? <Preview token={token} taskID={id}/> : <Notice>QC 数值资产保持原有 Zarr 契约；对照图在下游“对照图”任务中查看。</Notice>}</> : <Empty>尚无已登记产物。提交阶段中断时可先尝试“恢复结果登记”，不要直接重跑算法。</Empty>}</div>}
 </>}
