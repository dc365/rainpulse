...
it('defaults to all day and filters inclusive Beijing time without selecting or recomputing', () => {
  ...
  render(<HistoryPicker cycles={cycles} selectedID="0" onSelect={onSelect} />)
  fireEvent.click(screen.getByRole('button', {name: /历史案例 · 北京时间/}))
  expect(screen.getByRole('button', {name:'08:05'})).toBeTruthy()
  fireEvent.change(screen.getByLabelText('开始时间'), {target:{value:'08:10'}})
  expect(screen.queryByRole('button', {name:'08:05'})).toBeNull()
  expect(screen.getByRole('button', {name:'08:10'})).toBeTruthy()
