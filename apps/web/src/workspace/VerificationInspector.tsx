...
export function VerificationInspector({ detail, algorithm, validTime, point, onClear }: {
  ...
  return <section className="verification-inspector" aria-label="同格点检验对照">
    <div className="verification-context"><strong>原生时效对照</strong><span>{matched} 个实况匹配时效 · 固定起报</span>
      <label>事件：雨强 &gt; <select aria-label="检验雨强阈值" value={threshold} onChange={event => setThreshold(Number(event.target.value))}>
