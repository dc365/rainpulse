...
38 LK runs and all 38 12-member STEPS cycles; gaps remain explicit rather than
being filled into synthetic model input. Reprocessed analysis, QPE and NowcastInput artifact paths now include
...
on an official MRMS sample. An authorized Qwen3.8 maintenance window also passes
four-member GPU inference with zero repeat delta, about 1.06 GB peak allocated
memory and 0.153-second warm latency. The fixed `/opt` weight URI and dedicated
offline input/output contracts now run through a cached long-lived worker; a
full GPU object-chain test produced a validated 52.47 MB artifact and the
installed systemd process passed its NATS-backed health check. The unit remains
disabled outside approved GPU windows, and Qwen3.8 was restored healthy after
...
RP-027 closes the presentation loop for that frozen evidence without changing
the result. The algorithm-verification API now discriminates deterministic
spatial runs from probabilistic ensemble runs. The Web presents conclusion,
same-slice model scores, CRPS/Brier skill, coverage and GPU/runtime evidence,
while deliberately omitting deterministic FSS selectors and maps for RP-026.
...
The 105 holdout replay completed 50/50 issues with zero failures, 3,600 map
...
in `docs/RP033_超阈值概率空间地图实施记录.md`. The 105 replay completed all 50
frozen holdout issues without failure and atomically published 9,000 validated
