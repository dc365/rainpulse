...
不证明真实降水预报技巧。React 短临地图、五分钟时间轴、单点曲线、区域统计、
...
- 地图固定使用福州 EPSG:4326 网格 `118–123°E`、`25–27°N`，透明产品 PNG 与
  GSHHG 2.3.7 高精度海岸线在相同经纬坐标中叠加，不依赖在线地图服务。
- 支持 24 个五分钟时效、T+60/T+120 累计量切换和冻结的业务降水色标。
- 地图点击与方向键均可选择格点；单点面板展示 24 时效雨强曲线、当前置信度及
  T+30/T+60/T+120 关键值。
...
- 前端功能基线 `f9ed4c9` 编译为 Linux/amd64 Web 二进制和 Vite 静态产物，只重建
...
- 服务器实际产品目录加载 run
  ...
  两个累计产品和三类分发资产均可访问。
- T+60 切换后当前时效、点置信度和区域统计同步更新；切换 2 小时累计后展示 T+120
  及对应 NetCDF、COG、PNG 资产。
- 地图点击后经纬点查询更新，方向键可按 `0.01°` 移动；闽江口预设 bbox 返回
  2,806 个有效格点、0 个缺测格点。
- 1440、768、375、320 px 实际浏览器检查通过；375 和 320 px 页面无全局横向
  溢出，浏览器控制台为 0 错误、0 警告。
- 截图证据位于 `output/playwright/rp015-nowcast-105-1440.png` 和
  `output/playwright/rp015-nowcast-105-375.png`，不作为运行时发布资产。
