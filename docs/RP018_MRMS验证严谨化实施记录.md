...
RP-018 跨区域报告中，同一目标公里尺度可能因纬度和网格间距分别落到 9×9、
11×11 等不同奇数窗口；控制面会把相同 `window_target_km` 合并成一个 Web
选项，并把其中一个像素值作为稳定内部选择键，查询时再按目标公里数匹配当前
案例的实际窗口。旧 RP-016 报告仍按冻结像素窗口精确查询。
...
控制面读取 RP-018 报告时，以 `summary.json` 明确声明的
`metrics_truth_domain.csv` 作为 Web 评分数据源，不再把共同有效域指标当作主展示。
对比模型按钮由候选模型和 `skill_summary.comparisons` 中的基线生成，因此包含
持续性、LK 整场平移和独立相位相关；`lk_native_10min` 仍保留在指标接口中供
时间步长敏感性分析使用，但不混入同一业务时间步长下的基线按钮。未声明固定
实况域文件的旧 RP-016 报告继续读取 `metrics.csv`。
...
## 105 服务器全量验收（2026-08-29）

冻结配置 `rp018-mrms-v1` 在 105 服务器完成 `rp018-full-v1` 全量回算：

- 53/53 个起报完成，0 个失败；255 个依赖帧通过完整一致性检查；
- 共同有效域 57,240 行，固定实况域 95,400 行；
- 53 个不可变地图包、3,180 个图层，包含实况、LK、持续性、LK 整场平移和
  独立相位相关平移；
- 13 个 LK 运动回退，13 个独立相位相关回退，均显式记录；
- 运行指纹记录算法提交
  `b96a81da35d27c7eac7328c06bb3dbf4c05b45d3` 和配置 SHA-256
  `a05dc4efcc9985761b6d62f4a04e451f22e61e839000ec2b155bfaabe19a122a`；
- 单起报总耗时 P50/P95/最大值为 11.702/12.102/12.438 秒，核心预报加评分为
  6.504/6.644/7.167 秒；整进程峰值 RSS P50/P95/最大值约为
  849/974/983 MiB；
- `runtime_metrics.csv` 为 53 行，累积降水指标为 13,250 行；
- Web/API 已使用固定实况域指标，四个同时间步长模型可切换，原生 10 分钟 LK
  保留在指标接口中；相位相关 PNG 的尺寸、摘要和清单一致性通过接口读取验收。

严格结论是 `skill_not_demonstrated`，不能沿用 RP-016 的 `lk_supported`：

- LK 相对持续性的近时效平均 FSS 增益在 1/5/10 mm/h 分别为
  +0.0273/+0.0261/+0.0422；相对 LK 整场平移为
  +0.0077/+0.0071/+0.0134，但候选 LK 在固定实况域的最低覆盖仅 0.826，低于
  预声明 0.95 门槛；
- 独立相位相关对照中，Midwest 案例为负，四案例置信区间跨零；它不是 LK
  运动场的同源变体，因此该结果应保留而不能为了通过门槛删除；
- 70–120 分钟 LK 仍优于持续性，但最低覆盖降至 0.683，不能据此宣称远时效稳定；
- 5 分钟因果适配 LK 相对原生 10 分钟 LK 的平均 FSS 差为 -0.0467，说明时间步长
  适配会显著改变当前结果，进入留出集前必须单独解释或改进；
- 固定实况域相对共同域使 LK 有限 FSS 样本平均下降约 0.0012，证明共同域存在
  轻微乐观偏差；
- 湿案例、10 km 尺度的 0–1h/0–2h 累积 LK 平均 MAE 为 1.210/2.459 mm，
  平均偏差为 -0.059/-0.110 mm，当前总体略偏少。

该结论是严格门禁按设计工作，不是回算故障。下一步不得调低门槛或删除强基线来
追求“通过”，应先拆分平移边界导致的不可避免覆盖损失和算法内部缺测，再冻结
处理原则，随后使用未参与调参的月份做留出验证。
