...
- 真实官方后端和 105 CPU 数值冒烟已完成，GPU 验收因测试卡当前被其他服务占满而保持
  关闭。

当前已经完成配置契约、官方双通道数组适配、真实权重加载、四成员随机推理、可重复性
修复和 CPU 数值冒烟。尚未完成 GPU 资源/时延验收、MRMS 历史回算和技巧对比。
...
GPU 加载测试没有通过：当时 85,651 MiB 中仅剩约 435 MiB，显存由既有的两个
`llama-server`、VLLM 和语音服务占用。RainPulse 未停止这些非本项目进程；因此这是
外部资源阻塞，不是 GPU 数值验收通过，也不是模型 OOM 结论。

## 6. 下一步

1. 为 105 安排可释放至少 24 GB 显存的维护窗口或独占资源，不自动停止现有服务；
2. 完成四成员 GPU 推理、峰值显存、P50/P95 时延和同种子重复性验证；
3. 通过 GPU 验收后冻结权重运行 URI，再打开“离线单案例”门禁；
4. 从未用于 RP-021/RP-024 独立结论的新 MRMS 月份冻结 NowcastNet 开发集；
5. 另选未触碰的留出月份，与 persistence、translation、LK、STEPS 做共同有效域对比；
6. 只有福建连续 QC/QPE 数据到位后，才讨论实时并行灰度和本地适用性。
