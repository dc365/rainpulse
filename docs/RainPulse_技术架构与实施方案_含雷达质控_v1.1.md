...
> 2026-09-16 时效口径更新：运行链路改为 6 分钟间隔、未来 3 小时，并在同一时间轴加入过去 1 小时实况。本文下述旧 5 分钟/2 小时约定以 [实施记录](TIMELINE_6MIN_3H_20260916.md) 为准。NowcastNet 公开权重保留原生 10 分钟协议及 120 分钟上限，不虚构第三小时结果。
...
第一阶段使用 EPSG:4326 规则等经纬度格点；所有单雷达产品必须直接进入
注册的统一格点，不能先生成另一套业务投影网格再在产品阶段二次插值。
...
每部雷达使用完全相同的 `grid_id`、坐标数组和坐标摘要。增加雷达数量只增加
输入和贡献者数量，不改变融合后分析网格大小。
...
CRS: EPSG:4326
registration: point
lon: 118.00 ... 123.00，间隔 0.01°，共 501 点
lat:  25.00 ...  27.00，间隔 0.01°，共 201 点
dimension_order: lat × lon
```

起止经纬度表示格点中心且均包含在坐标数组中。透明图像的像素外边界为
`[117.995, 24.995, 123.005, 27.005]`，不能把格点中心边界直接当作图像外边界。

`0.01°` 不是固定一公里正方形。需要米制距离的极坐标插值、光流和运动场换算
必须使用版本化 WGS84 geodesic 网格尺度；禁止恢复单一 `resolution_m` 假设。
...
原生 DEM 保持约 30 米分辨率用于波束遮挡计算，不能先降采样到 0.01°。
海岸线派生陆海掩膜和近海辅助量。静态杂波图仍由每部雷达长期晴空基数据按
`sweep × azimuth × range` 统计，不能由 DEM 或海岸线代替。
