...
2026-09-25 已部署本地合并后的 Go/Web。两站首个体扫的全部 49 个扫层、147 张 raw/qc/flags PNG 均由新只读资产接口完成校验读取。
浏览器已验证两站切换、ZF505 非连续编号（含编号 10）和质控标记模式。图例使用 manifest 的实际分级，琥珀覆盖明确标注为待复核。
为验收连续切换，按现有预检查创建 00:06–00:12 UTC 的两个候选 QC 任务；作业 `81fa570b-9724-4514-8d8d-3e38d47c8689` 已 SUCCEEDED（2/2）。因此两站各有两个可对照体扫。
临时数据库测试容器在验证结束后停止并删除；没有使用业务表做测试夹具。回退备份保存在部署目录 `.build/rollback-x-workspace-84019e4`。
