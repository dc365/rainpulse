...
当前状态：第 2,819 步检查点已批准为正式起点，夜间调度已启用，首个正式窗口等待 20:00 启动
...
第 2,819 步演练检查点已获批准并完成晋级预检，正式演化训练从该步继续计数。训练固定使用生成该检查点的代码提交，主分支的文档或非训练变更不会改变恢复身份。夜间调度已启用，20:00 启动，07:45 请求优雅停止，08:00 前恢复 Qwen3.8。首窗结束后会自动暂停下一次启动，待报告、连续指标、输出检查点和共享服务恢复结果通过人工验收后再续训；独立留出继续关闭。
