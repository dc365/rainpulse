...
当前状态：首个正式窗口已验收，演化训练到第 243,046 步；第二个窗口尚未批准
...
首个正式窗口从第 2,819 步恢复到第 243,046 步，完成 240,227 步。全部 243,046 行指标连续且为有限值，最终检查点通过哈希、状态指纹和 CPU 回读验收，Qwen3.8 已恢复，Qwen3.6 没有停止。停机收尾曾因 `ExecStopPost` 内嵌套控制 timer 被 systemd 标记为 timeout，但训练报告、检查点和共享服务恢复均已在超时前完成；该嵌套调用已移除并通过无 GPU 复验。两个 timer 均保持关闭，第二窗需要一次性启动许可。剩余 56,954 步按首窗平均吞吐预计约 2.59 GPU 小时，独立留出继续关闭。
