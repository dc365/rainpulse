...
候选实现基线：`e150cc9`（主线 `main`）。

这是可执行的候选引擎和评估工具，不是已经达到气象最优、已完成真实过程准入的声明。当前仅在 105 以 shadow/candidate 配置部署并完成抽样回放，默认业务配置没有切换，也未修改原始观测或既有成功产物。

## 2026-09-13 V3 联合测量与外围复核

新候选实施、同输入对照、冻结任务实验和审计见 [RFI_MULTIVARIATE_V3.md](RFI_MULTIVARIATE_V3.md)。
本版基于包含实际 V2 部署记录的 `1251ea8`；不覆盖 V2 配置或门级上下文修复。
本次源码交付不表示 V3 已部署，也不代表三组真实残留已通过验收。

## 2026-09-12 径向对象候选更新

最新实现说明与配置切换见 [RFI_OBJECTS_V2.md](RFI_OBJECTS_V2.md)。
`fujian-qc-rfi-objects-v2` 是新版本候选，修复缺背景、短缺口、原始矩可用性与时间证据接入。
下文保留首版记录；v2增加冻结任务离线回放、隔离状态与算法独立报告，未自动部署或准入。
