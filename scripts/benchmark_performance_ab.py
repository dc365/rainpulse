...
from rainpulse_algo.multiband.preview_sampling import (
    prepare_polar_sampling,
    render_polar_sampling,
)
...
def reference():
    path = (
        ...
        / "algorithms/tests/performance_ab_20260926/references/legacy.py"
    )
...
def old_candidate(f, echo, snr, cfg, ranges):
    ...
    supported = (
        minimum_filter1d(echo.astype(np.uint8), size=n, axis=1, mode="nearest") == 1
    )
    return (
        echo
        & supported
...
def run(repeats):
    ...
    core = rng.random(labels.shape) < 0.5
    prop = ~core
    counts = np.bincount(labels.ravel())
    seeds = np.bincount(labels[core], minlength=len(counts))
    sizes = counts[labels].astype("u4")
    fractions = np.divide(
        seeds, counts, out=np.zeros_like(counts, dtype=float), where=counts > 0
    )[labels].astype("f4")
    def a():
        return legacy.loop_validate(labels, sizes, fractions, core, prop)
    def b():
    ...
    grouped = {
        **paired(a, b, repeats),
        ...
        "objects": 1500,
        "checks_passed": True,
        "scope": "count/fraction checks only; not full validator",
    }

    shape = (360, 2000)
    z = rng.uniform(-15, 50, shape).astype("f4")
    echo = rng.random(shape) > 0.001
    z[~echo] = np.nan
    snr = np.ones(shape, bool)
    cfg = SimpleNamespace(
        phase_window_m=975.0, rho_candidate_max=0.75, texture_candidate_db=8.0
    )
    ranges = np.arange(shape[1]) * 75.0 + 37.5
    candidates = []
    for active in (0, 36, 360):
        rho = np.full(shape, 0.99, "f4")
        rho[:active] = 0.7
        f = {"DBZH": z, "RHOHV": rho, "WEATHER_PROTECTED_MASK": np.zeros(shape, "u1")}
        def old():
            return old_candidate(f, echo, snr, cfg, ranges)
        def new():
            return nonmet_candidate(f, echo, snr, cfg, ranges)
        np.testing.assert_array_equal(old(), new())
        candidates.append(
            {
                **paired(old, new, repeats),
                "active_rays": active,
                "total_rays": 360,
                "equal_mask": True,
    ...
    az = np.arange(360.0)
    el = np.full(360, 0.5)
    ranges = np.arange(2000.0) * 75.0 + 37.5
    ...
    def old_images():
        ...
        for mapping in (None, sampling):
            for field, actions in ((z, None), (qc, None), (z, action)):
                result.append(
                    legacy.polar_quicklook(
                        az,
                        ranges,
                        field,
                        size=720,
                        map_sampling=mapping,
                        elevation_deg=el,
                        actions=actions,
                    )
                )
    ...
    def new_images():
        result = []
        for mapping in (None, sampling):
            plan = prepare_polar_sampling(
                az, ranges, size=720, map_sampling=mapping, elevation_deg=el
            )
            for field, actions in ((z, None), (qc, None), (z, action)):
                result.append(
                    render_polar_sampling(
    ...
    assert before == after
    images = {
        **paired(old_images, new_images, repeats),
        "images": 6,
        "size": [720, 720],
        "png_bytes_equal": True,
        "png_sha256": [hashlib.sha256(x).hexdigest() for x in after],
        "scope": "two sampling plans + six colors/PNG encodes; excludes GEOD grid, QC and I/O",
    }
    return {
        "base_commit": BASE,
        "synthetic": True,
        "real_site_test": False,
        "repetitions": repeats,
        "warmup_runs": 1,
        "group_validation": grouped,
        "x_candidate": candidates,
        "six_png": images,
    }
...
def main():
    ...
    try:
        ...
        result["runtime"] = {
            ...
            "libraries": {
                k: installed(k) for k in ("numpy", "scipy", "numba", "pyproj")
            },
