...
def field_metrics(field: np.ndarray, truth: np.ndarray | None) -> dict:
    ...
    seams = np.concatenate(
        [dx[:, np.array([96, 192, 288, 384, 480]) - 1].ravel(), dy[95].ravel()]
    )
    ...
    result = {
        ...
        "old_seam_mean_jump_mm_h": float(np.mean(finite_seams))
        ...
        "all_grid_mean_jump_mm_h": float(
            np.nanmean(np.concatenate([dx.ravel(), dy.ravel()]))
        ),
    }
    if truth is not None:
        ...
        result["mae_mm_h"] = (
            float(np.mean(np.abs(field[common] - truth[common])))
            if common.any()
            else None
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--halo-only",
        action="store_true",
        help="Compare identical original tile predictions with a narrow output halo",
    )
    ...
    runtime = _load_runtime(
        *(str(p) for p in paths),
        str(root / "runtime/nowcastnet/official-v1"),
        args.device,
    )
    # Same batch schedule in both arms; no GPU service or live queue is modified.
    runtime = replace(
        runtime, task=replace(runtime.task, gpu_batch_size=args.batch_size)
    )
    ...
    while True:
        ...
        if cursor:
            query["cursor"] = cursor
        page = fetch_catalog(
            args.api_base + "/api/v1/analysis-cycles?" + urlencode(query),
            timeout_seconds=30,
        )
    ...
    def capture_tiles(values, *, output_shape, **_worker_policy):
    ...
    for version in (
        ["v1", "v1-halo"]
        if args.halo_only
        else ["v1", "v2", "v2-sharp", "v2-rain-midpoint", "v3", "v3-rain-midpoint"]
    ):
        atlas_path = (
            config
            / "nowcast"
            / f"fujian-nowcastnet-tile-atlas-{version.split('-')[0]}.yaml"
        )
        atlas = load_tile_atlas(atlas_path)
        candidate = replace(
            runtime,
            atlas=atlas,
            task=replace(runtime.task, tile_atlas_version=atlas.atlas_version),
        )
        print(f"starting {version}, {len(atlas.tiles)} tiles", flush=True)
        started = time.monotonic()
        if version in ("v1-halo", "v2-sharp", "v2-rain-midpoint", "v3-rain-midpoint"):
            native, valid = stitch_member_tiles(
                cached_tiles,
                output_shape=atlas.grid_shape,
                weight_power=2 if version == "v2-sharp" else 1,
                trusted_halo=16 if version == "v1-halo" else 0,
            )
            ...
            with patch(
                "rainpulse_algo.nowcast.nowcastnet_shadow_worker.stitch_member_tiles",
                capture_tiles,
            ):
        ...
        report["variants"][version] = {
            ...
            "blend_policy": "raised-edge-squared-v1"
            if version == "v2-sharp"
            else "raised-edge-v1",
            ...
            "metrics": {
                str(lead): field_metrics(mean[lead // 5 - 1], truth.get(lead))
                for lead in leads
            },
        }
    ...
    for lead in leads:
        ...
        common = np.logical_and.reduce(
            [np.isfinite(value[index]) for value in arrays.values()]
        )
        if lead in truth:
            common &= np.isfinite(truth[lead])
        for version, value in arrays.items():
            report["variants"][version].setdefault("paired_metrics", {})[str(lead)] = (
                field_metrics(np.where(common, value[index], np.nan), truth.get(lead))
            )
    (args.output / "report.json").write_text(
        json.dumps(report, indent=2, allow_nan=False)
    )
    ...
    for row, version in enumerate(arrays):
        for col, lead in enumerate(leads):
            ax = axes[row, col]
            im = ax.imshow(
                arrays[version][lead // 5 - 1], origin="lower", cmap=cmap, norm=norm
            )
