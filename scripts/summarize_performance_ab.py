...
def summarize(lines, maximum_records=10000):
    ...
    for line in lines:
        ...
        for path, value in obj.get('stages', {}).items():
            ...
            if path not in stages and len(stages) >= 2048:
                raise ValueError('stage identity limit reached; split versions/workloads')
            item = stages.setdefault(path, {'calls':0,'inclusive_ms':0.,'self_ms':0.,'incomplete_self_records':0,'errors':0})
    ...
    return {'schema':'rainpulse.performance-ab-summary-v1','trace_records':total,
            'ignored_lines':ignored,'call_status_counts':dict(statuses),
            'root_wall_ms':{path:{'count':len(v),'p50':percentile(v,.5),'p95':percentile(v,.95),
                       'p99':percentile(v,.99),'maximum':max(v)} for path,v in roots.items()},
            'stages':dict(sorted(stages.items(), key=lambda pair:pair[1]['self_ms'], reverse=True)),
