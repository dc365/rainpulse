...
const program = ts.createProgram([path.join(root, 'apps/web/src/admin/model.ts')], options)
...
const result = spawnSync(process.execPath, ['--test', path.join(root, 'tests/operations/model.test.cjs')], {
  stdio: 'inherit', env: { ...process.env, OPS_MODEL_JS: path.join(output, 'model.js') },
})
