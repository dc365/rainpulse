...
func serve(ctx context.Context, store *postgresstore.Store, bus *messaging.JetStream, service *orchestration.Service) error {
	...
	launch := func(run func()) { tasks.Add(1); go func() { defer tasks.Done(); run() }() }
	...
	if ingestSettings != nil {
		...
		launch(func() { radarIngestLoop(ctx, scanner, *ingestSettings, service) })
	}
	...
	if pipelineSettings != nil {
		launch(func() { newPipelinePlanner(*pipelineSettings, store, service).Run(ctx) })
	}
	...
	launch(func() { dispatchLoop(ctx, service, bus) })
	launch(func() { nowcastNetRetentionLoop(ctx, store) })
