...
func (b *OperationsBuilder) buildMultiBand(ctx context.Context, s operations.Selection) ([]operations.Spec, []operations.Check, error) {
	...
	checks := []operations.Check{{Code: "multiband_scope", State: "WARN", Message: "S复用已登记QC；X采用独立候选策略；同高度质量选源后生成组合反射率。仅候选，不改变QPE/预报/默认展示。"},
	...
	for _, snap := range snapshots {
		id := operations.NewID()
		payload := map[string]any{"mode": s.Preset, "product_id": product, "network_sha256": n.SHA256, "analysis_time": snap.AnalysisTime,
		...
		specs = append(specs, operations.Spec{ID: id, Kind: "multiband", Name: fmt.Sprintf("%s · %s · %s", s.Preset, product, snap.AnalysisTime.Format("01-02 15:04 UTC")), InputURIs: uris, Request: operations.JSON(request),
			Identity: operations.Identity{Kind: "multiband", Files: map[string]string{"RAINPULSE_MULTIBAND_CONFIG": n.SHA256, "RAINPULSE_QC_FLAG_DEFINITIONS": operations.Digest(flagBytes)}, Versions: map[string]string{"multiband_contract": "rainpulse.multiband.v1", "network_release": n.Release}}})
	}
	state := "PASS"
	if len(specs) == 0 {
		state = "BLOCK"
	}
	checks = append(checks, operations.Check{Code: "multiband_tasks", State: state, Message: fmt.Sprintf("冻结%d个一分钟产品或X单站任务；不对S重复运行QC。", len(specs))})
