...
func Main() {
	...
	if err != nil {
		slog.Error("initialize ingest", "error", err)
		os.Exit(1)
	}
	defer pool.Close()
	if err := Run(ctx, platformRuntime, service); err != nil {
		slog.Error("ingest stopped", "error", err)
		os.Exit(1)
	}
}

func Run(ctx context.Context, platformRuntime bdpruntime.Runtime, service *orchestration.Service) error {
	...
	if manifestPath == "" {
		...
		return fmt.Errorf("RAINPULSE_RADAR_INGEST_MANIFEST is required")
	}
