...
func TestNowcastNetShadowInputTimesSelectsFifteenExactSixMinuteFrames(t *testing.T) {
	issue := time.Date(2026, 8, 28, 2, 25, 0, 0, time.FixedZone("CST", 8*60*60))
	values := nowcastNetShadowInputTimes(issue)
	if len(values) != 15 {
		t.Fatalf("input frame count = %d, want 15", len(values))
	}
	for index, value := range values {
		want := issue.UTC().Add(time.Duration(-84+6*index) * time.Minute)
		if !value.Equal(want) {
